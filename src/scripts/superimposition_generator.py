...
def load_TM_alignment_data(input_fname_base, alignment_tool, alignment_dir, graphs_and_pickles_dir, alignment_fname, clusters, previous_graph_file_reused):

    alignment_data_fname = os.path.join(graphs_and_pickles_dir, 'alignment_data_' + input_fname_base + '_' + alignment_tool + '.pickle2')
    if (sys.version_info >= (3, 0)):
        alignment_data_fname = os.path.join(graphs_and_pickles_dir, 'alignment_data_' + input_fname_base + '_' + alignment_tool + '.pickle3')
...
def load_alignment_data(input_fname_base, alignment_tool, alignment_dir, graphs_and_pickles_dir, alignment_fname, clusters, previous_graph_file_reused):
    ...
    # print('Check graph data')
    # print(cluster_alignment_data['E-loop'][strToNode('4V88_A6:626-630_967-971')][strToNode('1MFQ_A:78-82_93-97')])
    # print(cluster_alignment_data['E-loop'][strToNode('1MFQ_A:78-82_93-97')][strToNode('4V88_A6:626-630_967-971')])
    # sys.exit()
                ...
                if lines[line_index].startswith('#  Aligning'):
                    ...
                    for c_id, node_dict in cid_nodelist_pair:
                        if node2 in node_dict:
                            ...
                            zscore = cluster_alignment_data[c_id][node1][node2][2]
                            # prev_score = cluster_alignment_data[c_id][node1][node2][7]
                            # if prev_score < score:
                            cluster_alignment_data[c_id][node1][node2] = (t1, t2, zscore, cr1, cr2, aln1, aln2, score)
                            cluster_alignment_data[c_id][node2][node1] = (t2, t1, zscore, cr2, cr1, aln2, aln1, score)
                            # break
...
def generate_rmsd_data(input_fname_base, alignment_tool, partial_pdbx_dir, graphs_and_pickles_dir, alignment_data, clusters, loop_list, previous_graph_file_reused):

    rmsd_data_fname = os.path.join(graphs_and_pickles_dir, 'rmsd_data_' + input_fname_base + '_' + alignment_tool + '.pickle2')
    if (sys.version_info >= (3, 0)):
        rmsd_data_fname = os.path.join(graphs_and_pickles_dir, 'rmsd_data_' + input_fname_base + '_' + alignment_tool + '.pickle3')
...
def load_alignment_and_rmsd_data(clusters, loop_list, input_fname_base, alignment_tool, partial_pdbx_dir, alignment_dir, graphs_and_pickles_dir, previous_graph_file_reused):
    graph_fname = os.path.join(graphs_and_pickles_dir, input_fname_base + '_' + alignment_tool + '.z.graph')

    if alignment_tool == 'TMalign':
        alignment_data = load_TM_alignment_data(input_fname_base, alignment_tool, alignment_dir, graphs_and_pickles_dir, graph_fname, clusters, previous_graph_file_reused)
    else:
        alignment_data = load_alignment_data(input_fname_base, alignment_tool, alignment_dir, graphs_and_pickles_dir, graph_fname, clusters, previous_graph_file_reused)
    ...
    rmsd_data_dict = generate_rmsd_data(input_fname_base, alignment_tool, partial_pdbx_dir, graphs_and_pickles_dir, alignment_data, clusters, loop_list, previous_graph_file_reused)
