...
def parse_TM_alignment_block(lines, line_index):
    ...
    seq_identity = seq_identity_text.strip().split('=')[1].strip()
...
def get_motif_family_fullname(family_name_short):
    if family_name_short in list(known_motif_shortcode.values()):
        family_name = list(known_motif_shortcode.keys())[list(known_motif_shortcode.values()).index(family_name_short)]
        return known_motif_fullname[family_name]
    else:
        return family_name_short
...
def load_loop_data(loop):
    ...
    loop_fn = os.path.join(loop_dir, loop + '.smf')
    fp = open(loop_fn)
    lines = fp.readlines()
    fp.close()
...
def filter_loops_containing_HETATM(families, partial_pdbx_dir):
    ...
    for family_id in families:
        ...
        filtered_loops = []
        for loop in loops:
            loop_fn = os.path.join(partial_pdbx_dir, loop + '.cif')
            fp = open(loop_fn)
            lines = fp.readlines()
            fp.close()
            has_HETATM = False
